...
function initialTextReplacement($input, initialvalue) {
	if ($input.val() != '' && $input.val() != initialvalue) {
		// field already has a value (from browser history for a refresh)
		return;
	}
	$input.val(initialvalue);
	$input.focus(function() {
		if ($.trim($input.val()) == initialvalue) {
			$input.val('');
		}
	});
	$input.blur(function() {
...
function unsetFormValue(form, name) {
	if (getFormElement(form, name)) {
		setFormValue(form, name, '');
	}
}

function relevanceSearch(start, count, currentTopic, currentTopicType) {
	var searchForm = getForm('searchForm');
	unsetFormValue(searchForm, 'ARTICLE_START');
	unsetFormValue(searchForm, 'ARTICLE_COUNT');
	unsetFormValue(searchForm, 'TOPIC_START');
	unsetFormValue(searchForm, 'TOPIC_COUNT');
	unsetFormValue(searchForm, 'GUIDED_HELP_START');
	unsetFormValue(searchForm, 'GUIDED_HELP_COUNT');
	unsetFormValue(searchForm, 'FORUM_START');
	unsetFormValue(searchForm, 'FORUM_COUNT');
	setFormValue(searchForm, 'RESULTS', 'RELEVANCE');
	setFormValue(searchForm, 'RELEVANCE_START', '' + start);
	setFormValue(searchForm, 'RELEVANCE_COUNT', '' + count);

	/* Specify topic to constrain search results to, if specified */
	if (currentTopic) {
		if (!currentTopicType)
			currentTopicType = 0;
		setFormValue(searchForm, 'BASIC_SEARCH_CURRENT_TOPIC_ID', '' + currentTopic);
		setFormValue(searchForm, 'BASIC_SEARCH_CURRENT_TOPIC_TYPE', '' + currentTopicType);
	}
	return basicSearchSubmit();
}

function contentTypeSearch(start, count, currentTopic, currentTopicType) {
	var searchForm = getForm('searchForm');
	unsetFormValue(searchForm, 'RELEVANCE_START');
	unsetFormValue(searchForm, 'RELEVANCE_COUNT');
	setFormValue(searchForm, 'RESULTS', 'CONTENT_TYPE');
	setFormValue(searchForm, 'ARTICLE_START', '1');
	setFormValue(searchForm, 'ARTICLE_COUNT', '10');
	setFormValue(searchForm, 'TOPIC_START', '1');
	setFormValue(searchForm, 'TOPIC_COUNT', '5');
	setFormValue(searchForm, 'GUIDED_HELP_START', '1');
	setFormValue(searchForm, 'GUIDED_HELP_COUNT', '5');
	setFormValue(searchForm, 'FORUM_START', '1');
	setFormValue(searchForm, 'FORUM_COUNT', '5');
...
function verify() {
	...
	if (expandAndValidate(searchFrm.searchString.value)) {
...
var advSearchFrm = document.getElementById("advanceSearchForm");
	if (attribList.length > 0) {
		...
		newCell.innerHTML = "<button type='button' class='ButtonClass' value='Go!' onclick=\"advancedSearchSubmit()\">"+searchButton+"</button>";
...
function submitFormUsingDropDown(cmdVal) {
	...
	if(sideLinksTopic.selectedIndex > -1) {
		selectedTopic = sideLinksTopic.options[sideLinksTopic.selectedIndex].value;
	}
...
function simulateButtonClicks(searchButton, removeCr) {
	var searchFrm = document.getElementById("advanceSearchForm");
	if (userInputStr == null || userInputStr == 'undefined') return;
	var criterions = userInputStr.split('#');
	if (criterions == null || typeof(criterions) == 'undefined') return;

	for (var i=0; i<criterions.length; i++) {
		var cr = criterions[i];
		var attribArray = cr.split(":");
		if (attribArray!=null && typeof(attribArray)!= 'undefined'
			&& attribArray[0] != '' && attribArray[1] != '' && attribArray[2] != '' && attribArray[3] != ''
...
function runSearch(newSearch) {

	var sEl = document.getElementById('searchString');
	if (sEl)
	{	// fix to prevent empty search
		var sString = sEl.value;
		if (sString.trim() == "") {
			return false;
		}
	}
	...
	var start = 1;
	var count = commonVariables.articlesPerPage;

	if (newSearch) {
		$('#BASIC_SEARCH_CURRENT_TOPIC_ID').remove();
		$('#BASIC_SEARCH_CURRENT_TOPIC_TYPE').remove();
		var searchForm = document.getElementById("searchForm");
		if ($('#searchTopic').is(':visible')) {
			setFormValue(searchForm, 'BASIC_SEARCH_CURRENT_TOPIC_ID', '' + $('#searchTopic').val());
			setFormValue(searchForm, 'BASIC_SEARCH_CURRENT_TOPIC_TYPE', '' + 0);
		}
	}

	if (searchType == 'CONTENT_TYPE') {
		return contentTypeSearch(start, count);
	}
	else {
		return relevanceSearch(start, count);
	}
}

function topicSearch() {
	var searchForm = document.getElementById("searchForm");
	setFormValue(searchForm, 'searchString', $('#topicSearchString').val());
	setFormValue(searchForm, 'BASIC_SEARCH_CURRENT_TOPIC_ID', '' + commonVariables.currentTopicId);
	setFormValue(searchForm, 'BASIC_SEARCH_CURRENT_TOPIC_TYPE', '' + commonVariables.currentTopicType);
	return runSearch(false);
}

function refineResults() {
	$('#searchRefinementModule').clone().hide().appendTo($('#searchForm'));
	return runSearch(false);
}

/*
This function would set the search form parameters and invoke basicSearchSubmit inorder to submit the request to server
*/
function basicSearchSubmitWithSearchParameter(searchString,start,pagesize)
{
	var searchFrm = document.getElementById("searchForm");
	searchFrm.searchString.value = searchString;
	if(typeof (searchFrm.RELEVANCE_START)!= "undefined" && typeof (searchFrm.RELEVANCE_COUNT)!= "undefined" && start != "undefined"  && pagesize != "undefined")
	{
		searchFrm.RELEVANCE_START.value = start;
		searchFrm.RELEVANCE_COUNT.value = pagesize;
	}
	return basicSearchSubmit();
}
